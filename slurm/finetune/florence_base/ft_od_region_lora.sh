#!/bin/bash -l

#SBATCH -A uppmax2020-2-2
#SBATCH -M snowy
#SBATCH -p node
#SBATCH -N 1
#SBATCH -t 3-00:00
#SBATCH -J finetune/florence_base/ft_od_region__lora
#SBATCH -o logs_uppmax/finetune/florence_base/ft_od_region__lora.out
#SBATCH -e logs_uppmax/finetune/florence_base/ft_od_region__lora.err
#SBATCH --gres=gpu:1
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=hoang-ha.pham.1833@student.uu.se


source activate /crex/proj/uppmax2024-2-24/hapham/envs/vlm
PROJECT_DIR="/proj/uppmax2024-2-24/hapham/vlm"
DATA_DIR="/proj/uppmax2024-2-24/hapham/data/riksarkivet"

cd $PROJECT_DIR

python pipelines/train/finetune_florence_od.py \
    --data-dir $DATA_DIR \
    --model-name florence_base__ft_od_region__lora \
    --num-train-epochs 10 \
    --max-train-steps 20000 \
    --batch-size 4 \
    --logging-interval 1000 \
    --use-lora true \
    --detect-class region
